...
/**
 * This class provides convenience access to many of the resources required by the workbench. The
 * class stores some images as descriptors, and some are stored as real Images in the registry. This
 * is a pure speed-space tradeoff. The trick for users of this class is that images obtained from
 * the registry (using getImage()), don't require disposal since they are shared, while images
 * obtained using getImageDescriptor() will require disposal. Consult the declareImages method to
 * see if a given image is declared as a registry image or just as a descriptor. If you change an
 * image from being stored as a descriptor to a registry image, or vice-versa, make sure to check
 * all users of the image to ensure they are calling the correct getImage... method and handling
 * disposal correctly.
 *
 * Images: - use getImage(key) to access cached images from the registry. - Less common images are
 * found by calling getImageDescriptor(key) where key can be found in IWorkbenchGraphicConstants
 *
 * This class initializes the image registry by declaring all of the required graphics. This
 * involves creating image descriptors describing how to create/find the image should it be needed.
 * The image is not actually allocated until requested.
 *
 * Some Images are also made available to other plugins by being placed in the descriptor table of
 * the SharedImages class.
 *
...
public class SQLImages
{
    ...
    /**
     * Declares a workbench image given the path of the image file (relative to the workbench
     * plug-in). This is a helper method that creates the image descriptor and passes it to the main
     ...
     * <p>
     * The workbench remembers the given image descriptor under the given name, and makes the image
     * available to plug-ins via {@link org.eclipse.ui.ISharedImages IWorkbench.getSharedImages()}.
     * For "shared" images, the workbench remembers the image descriptor and will manages the image
     * object create from it; clients retrieve "shared" images via
     * {@link org.eclipse.ui.ISharedImages#getImage ISharedImages.getImage()}. For the other,
     * "non-shared" images, the workbench remembers only the image descriptor; clients retrieve the
     * image descriptor via {@link org.eclipse.ui.ISharedImages#getImageDescriptor
     * ISharedImages.getImageDescriptor()} and are entirely responsible for managing the image
     * objects they create from it. (This is made confusing by the historical fact that the API
     * interface is called "ISharedImages".)
     * </p>
     *
    ...
    /**
     * Returns the image stored in the workbench plugin's image registry under the given symbolic
     * name. If there isn't any value associated with the name then <code>null</code> is returned.
     *
     * The returned Image is managed by the workbench plugin's image registry. Callers of this
     * method must not dispose the returned image.
     *
    ...
    /**
     * Returns the image descriptor stored under the given symbolic name. If there isn't any value
     * associated with the name then <code>null
     * </code> is returned.
     *
     * The class also "caches" commonly used images in the image registry. If you are looking for
     * one of these common images it is recommended you use the getImage() method instead.
     */
     ...
     * PROPBAN_ Enable toolbar ETOOL_ Disable toolbar DTOOL_ Local enabled toolbar ELCL_ Local
     * Disable toolbar DLCL_ Object large OBJL_ Object small OBJS_ View VIEW_ Product images PROD_
     * Misc images MISC_
     *
    ...
    // return
    // ImageDescriptor.createFromURL(SQLUIActivator.getDefault().getBundle().getEntry(ICONS_PATH +
    // relativePath));
    // }
}
