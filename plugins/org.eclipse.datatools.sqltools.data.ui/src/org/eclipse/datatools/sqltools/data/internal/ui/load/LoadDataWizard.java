...
public class LoadDataWizard extends Wizard
{
    ...
    private String EXTERNAL_LOAD_EXT_POINT = "org.eclipse.datatools.sqltools.data.ui.externalTableDataLoad"; //$NON-NLS-1$
    ...
    private Object getExternalLoad(Connection conn) {
		...
		if (conn != null)
		{
			try
			{
                ...
                IExtensionPoint extensionPoint = extensionRegistry.getExtensionPoint(EXTERNAL_LOAD_EXT_POINT);
                ...
                for (int numExt = 0; numExt < extensions.length; numExt++) {
                    ...
                    for (int config = 0; config < configElements.length; config++) {
                        String extensionVendor = configElements[config].getAttribute(EXTERNAL_LOAD_EXT_POINT_VENDOR);
                        if (thisVendor.equalsIgnoreCase(extensionVendor)) {
                            // vendor matched so far, check for version is at least this
                            String extensionVersion = configElements[config].getAttribute(EXTERNAL_LOAD_EXT_POINT_VERSION);
                            float ver = Float.parseFloat(extensionVersion);
                            if (thisMajorVersion + (thisMinorVersion * .1) >= ver) {
                                // use the external loader
                                externalExecutable = configElements[config].createExecutableExtension(EXTERNAL_LOAD_EXT_POINT_CLASS);
                            }
                        }
                    }
                }
