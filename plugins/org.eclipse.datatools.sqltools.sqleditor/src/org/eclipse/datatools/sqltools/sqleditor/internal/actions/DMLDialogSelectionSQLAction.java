...
public class DMLDialogSelectionSQLAction extends Action  implements ISelectionChangedListener, IPageUpdate, IUpdate
{
     ...
     * (non-Javadoc)
     * 
     * @see org.eclipse.jface.viewers.ISelectionChangedListener#selectionChanged(org.eclipse.jface.
     * viewers.SelectionChangedEvent)
     */
    ...
    /*
     * protected HashMap getVariableDeclarations() { int start = 0; int length = 0; if (_selection
     * == null) { _selection = (ITextSelection) _sqlEditor.getSelectionProvider().getSelection(); }
     * //get the offset of the selection if (_selection != null && !_selection.isEmpty()) { start =
     * _selection.getOffset(); length = _selection.getLength(); if (length < 0) { length = -length;
     * start -= length; } } //when user selects a range int offset = length > 0 ? start + 1 : start;
     * 
     * IDocument document =
     * _sqlEditor.getDocumentProvider().getDocument(_sqlEditor.getEditorInput()); ParsingResult
     * result = _sqlEditor.getParsingResult();
     * 
     * HashMap variables = null; if (result != null) { variables = result.getVariables(document,
     * offset); HashMap sp_params = result.getParameters(document, offset);
     * variables.putAll(sp_params); }
     * 
     * return variables; } protected SQLEditor getEditor() { return _sqlEditor; }
     */
