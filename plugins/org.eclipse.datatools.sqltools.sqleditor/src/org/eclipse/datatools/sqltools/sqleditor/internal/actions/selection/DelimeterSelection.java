...
public class DelimeterSelection implements ISQLSelection
{
    ...
    /**
     * We need to remove last terminator from selected text. This method calculates the appropriate
     * offset after remove the last terminator.
     *
     * @param text
     * @param lastOffset
     ...
     */
    // private int removeTerminator(String text, int startOffset, int lastOffset, int
    // usedTerminatorIndex)
    // {
    ...
    // {
    // if (lastWord.equalsIgnoreCase(term.toLowerCase()) ||
    // lastWord.equalsIgnoreCase(term.toUpperCase()))
    // {
    // isRemoved = true;
    // endOffset = lastStart;
    // }
    ...
    /**
     * Test whether the passed-in terminator need take up whole word or not. Here we use an simple
     * policy: Terminators with char started or ended need to take up a whole word.
     *
     * @param terminator
     ...
     */
    // private int getOffsetStepByWord(String text, int offset, boolean forward, String[]
    // terminators)
    // {
    ...
    // if (terminators[i].toUpperCase().equals(nextWord) ||
    // terminators[i].toLowerCase().equals(nextWord))
    // {
    // findTerm = true;
    ...
    // int curOffset = ((ITextSelection)
    // _sqlEditor.getSelectionProvider().getSelection()).getOffset();
    // int delta = 0;
    ...
    protected boolean isCode(int offset) {
        ...
        try {
            ...
            for (int tempOffset = offset - 1; tempOffset <= offset; tempOffset++) {
                ...
                contentType = ((IDocumentExtension3) document).getContentType(SQLPartitionScanner.SQL_PARTITIONING, tempOffset, false);

                if (contentType == null)
                    return false;

                if (contentType.equals(SQLPartitionScanner.SQL_COMMENT) || contentType.equals(SQLPartitionScanner.SQL_MULTILINE_COMMENT) || contentType.equals(SQLPartitionScanner.SQL_DOUBLE_QUOTES_IDENTIFIER) || contentType.equals(SQLPartitionScanner.SQL_STRING)) {
    ...
    //
    // // for possible spaces
    // while (offset >= 0 && offset < text.length() && (!isCode(offset) ||
    // isWordDelimiter(text.charAt(offset))))
    // {
    // offset += step;
    // }
    //
