...
public class TableCursor extends Canvas
{
    ...
    Listener         _tableListener, _resizeListener, _disposeItemListener, _disposeColumnListener;
    ...
    public TableCursor(Table parent, int style)
    {
        ...
        ;
        _table.addListener(SWT.FocusIn, _tableListener);
        _table.addListener(SWT.MouseDown, _tableListener);

        _disposeItemListener = new Listener()
        {
            public void handleEvent(Event event)
            {
                _row = null;
        ...
        ;
        _disposeColumnListener = new Listener()
        {
            public void handleEvent(Event event)
            {
                _row = null;
        ...
        ;
        ScrollBar hBar = _table.getHorizontalBar();
        if (hBar != null)
        {
            hBar.addListener(SWT.Selection, _resizeListener);
        }
        ScrollBar vBar = _table.getVerticalBar();
        if (vBar != null)
        {
            vBar.addListener(SWT.Selection, _resizeListener);
        }
    }
    ...
    {
        _table.removeListener(SWT.FocusIn, _tableListener);
        _table.removeListener(SWT.MouseDown, _tableListener);
        if (_column != null)
        {
            _column.removeListener(SWT.Dispose, _disposeColumnListener);
            _column.removeListener(SWT.Move, _resizeListener);
            _column.removeListener(SWT.Resize, _resizeListener);
            _column = null;
        }
        if (_row != null)
        {
            _row.removeListener(SWT.Dispose, _disposeItemListener);
            _row = null;
        }
        ScrollBar hBar = _table.getHorizontalBar();
        if (hBar != null)
        {
            hBar.removeListener(SWT.Selection, _resizeListener);
        }
        ScrollBar vBar = _table.getVerticalBar();
        if (vBar != null)
        {
            vBar.removeListener(SWT.Selection, _resizeListener);
        }
    }
    ...
    void keyDown(Event event)
    {
        ...
        int rowIndex = _table.indexOf(_row);
        int columnIndex = _column == null ? 0 : _table.indexOf(_column);
        switch (event.keyCode)
        {
            case SWT.ARROW_UP:
                ...
                break;
            case SWT.ARROW_DOWN:
                setRowColumn(Math.min(rowIndex + 1, _table.getItemCount() - 1), columnIndex, true);
                break;
            case SWT.ARROW_LEFT:
            case SWT.ARROW_RIGHT: 
                {
                    ...
                    if (columnCount == 0)
                    break;
                    int[] order = _table.getColumnOrder();
                    int index = 0;
            ...
            case SWT.PAGE_UP: 
                {
                    ...
                    if (index == rowIndex)
                    {
                        Rectangle rect = _table.getClientArea();
                        TableItem item = _table.getItem(index);
                        Rectangle itemRect = item.getBounds(0);
                        rect.height -= itemRect.y;
                        int height = _table.getItemHeight();
    ...
    void paint(Event event)
    {
        ...
        int x = 0;
        Point size = getSize();
        Image image = _row.getImage(columnIndex);
        if (image != null)
        {
        ...
        String text = _row.getText(columnIndex);
        if (text != "") 
        {
            //$NON-NLS-1$
            Rectangle bounds = _row.getBounds(columnIndex);
            ...
            {
                //$NON-NLS-1$
                if (_table.getColumnCount() == 0 || columnIndex == 0)
                {
                    x += 2;
                }
                else
                {
                    int alignmnent = _column.getAlignment();
                    switch (alignmnent)
                    {
                        case SWT.LEFT:
                            x += 6;
    ...
    void tableMouseDown(Event event)
    {
        ...
        Rectangle clientRect = _table.getClientArea();
        int columnCount = _table.getColumnCount();
        int maxColumnIndex = columnCount == 0 ? 0 : columnCount - 1;
        int start = _table.getTopIndex();
        int end = _table.getItemCount();
    ...
    {
        TableItem item = row == -1 ? null : _table.getItem(row);
        TableColumn col = column == -1 || _table.getColumnCount() == 0 ? null : _table.getColumn(column);
        setRowColumn(item, col, notify);
    }

    void setRowColumn(TableItem row, TableColumn column, boolean notify)
    {
        // Alway do not set the first column focus.
        if (_table.indexOf(column) == 0)
        {
            column = _table.getColumn(_table.indexOf(column) + 1);
        }
        ...
        {
            this._row.removeListener(SWT.Dispose, _disposeItemListener);
            this._row = null;
        }
        if (this._column != null && this._column != column)
        {
            this._column.removeListener(SWT.Dispose, _disposeColumnListener);
            this._column.removeListener(SWT.Move, _resizeListener);
            this._column.removeListener(SWT.Resize, _resizeListener);
            this._column = null;
        }
        if (row != null)
        {
            ...
            {
                this._row = row;
                row.addListener(SWT.Dispose, _disposeItemListener);
                _table.showItem(row);
            }
            if (this._column != column && column != null)
            {
                this._column = column;
                column.addListener(SWT.Dispose, _disposeColumnListener);
                column.addListener(SWT.Move, _resizeListener);
                column.addListener(SWT.Resize, _resizeListener);
                _table.showColumn(column);
            }
            int columnIndex = column == null ? 0 : _table.indexOf(column);
    ...
    void resize()
    {
        ...
        {
            int columnIndex = _column == null ? 0 : _table.indexOf(_column);
            setBounds(_row.getBounds(columnIndex));
        }
    }
    ...
    {
        checkWidget();
        return _column == null ? 0 : _table.indexOf(_column);
    }
    ...
    public void setSelection(int row, int column)
    {
        ...
        int columnCount = _table.getColumnCount();
        int maxColumnIndex = columnCount == 0 ? 0 : columnCount - 1;
        if (row < 0 || row >= _table.getItemCount() || column < 0 || column > maxColumnIndex)
        {
    ...
    public void setSelection(TableItem row, int column)
    {
        ...
        setRowColumn(_table.indexOf(row), column, false);
    }
}
