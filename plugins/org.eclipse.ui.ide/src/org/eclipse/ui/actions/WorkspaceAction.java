...
/**
 * The abstract superclass for actions which invoke commands implemented in org.eclipse.core.* on a
 * set of selected resources.
 *
 * It iterates over all selected resources; errors are collected and displayed to the user via a
 * problems dialog at the end of the operation. User requests to cancel the operation are passed
 * along to the core.
 * <p>
 * Subclasses must implement the following methods:
 * <ul>
 * <li>invokeOperation - to perform the operation on one of the selected resources</li>
 * <li>getOperationMessage - to furnish a title for the progress dialog</li>
 * </ul>
 * </p>
 * <p>
 * Subclasses may override the following methods:
 * <ul>
 * <li>shouldPerformResourcePruning - reimplement to turn off
 * <li>updateSelection - extend to refine enablement criteria
 * <li>getProblemsTitle - reimplement to furnish a title for the problems dialog</li>
 * <li>getProblemsMessage - reimplement to furnish a message for the problems dialog</li>
 * <li>run - extend to
 * </ul>
 * </p>
 */
public abstract class WorkspaceAction extends SelectionListenerAction {
     ...
     * @param text the string used as the text for the action, or <code>null if there is no text
     ...
     * Creates a new action with the given text.
     *
     * @param provider the shell provider (for the modal progress dialog and error messages)
     * @param text the string used as the text for the action, or <code>null if there is no text
     * @since 3.4
     */
    ...
    /**
     * Runs <code>invokeOperation on each of the selected resources, reporting progress and fielding
     * cancel requests from the given progress monitor.
     * <p>
     * Note that if an action is running in the background, the same action instance can be executed
     * multiple times concurrently. This method must not access or modify any mutable state on
     * action class.
     *
     ...
     * <p>
     * The <code>WorkspaceAction implementation of this method returns a vague message (localized
     * counterpart of something like "The following problems occurred."). Subclasses may reimplement
     * to provide something more suited to the particular action.
     * </p>
     *
     ...
     * <p>
     * The <code>WorkspaceAction implementation of this method returns a generic title (localized
     * counterpart of "Problems"). Subclasses may reimplement to provide something more suited to
    ...
    /**
     * Returns the shell for this action. This shell is used for the modal progress and error
     * dialogs.
     *
     * @return the shell
     */
    ...
    /**
     * Performs this action's operation on each of the selected resources, reporting progress to,
     * and fielding cancel requests from, the given progress monitor.
     * <p>
     ...
     *             {@link #createOperation(IStatus[])} and provide an empty implementation for this
     *             method.
     *             </p>
     *
     ...
     *
     * @param resources the list of resources (element type: <code>IResource)
     * @param child the resource to check
     * @return <code>true if child is a descendent of any of the elements of <code>resources
     */
     ...
     * <code>shouldPerformResourcePruning.
     *
     * @param resourceCollection the list of resources (element type: <code>IResource)
     * @return the list of resources (element type: <code>IResource) after pruning.
    ...
    /**
     * The <code>CoreWrapperAction implementation of this <code>IAction method uses a
     * ProgressMonitorDialog to run the operation. The operation calls <code>execute (which, in
     * turn, calls <code>invokeOperation). Afterwards, any <code>CoreExceptions encountered while
     * running the operation are reported to the user via a problems dialog.
     * <p>
    ...
    /**
     * Returns whether this action should attempt to optimize the resources being operated on. This
     * kind of pruning makes sense when the operation has depth infinity semantics (when the
     * operation is applied explicitly to a resource then it is also applied implicitly to all the
     * resource's descendents).
     * <p>
     * The <code>WorkspaceAction implementation of this method returns <code>true. Subclasses should
     * reimplement to return <code>false if pruning is not required.
     * </p>
     *
     * @return <code>true if pruning should be performed, and <code>false if pruning is not desired
     *
     * @since 3.1
     */
    ...
    /**
     * The <code>WorkspaceAction implementation of this <code>SelectionListenerAction method ensures
     * that this action is disabled if any of the selected resources are inaccessible. Subclasses
     * may extend to react to selection changes; however, if the super method returns <code>false,
     * the overriding method should also return <code>false.
     */
    ...
    /**
     * Returns the elements that the action is to be performed on. By default return the selected
     * resources.
     * <p>
     * Subclasses may override this method.
     *
     * @return list of resource elements (element type: <code>IResource)
     */
     ...
     *
     * @param rule The rule to apply to the background job or <code>null if there isn't one.
     */
     ...
     *
     * @param rule The rule to apply to the background job or <code>null if there isn't one.
     * @param jobFamily a single family that the job should belong to or <code>null if none.
     *
     * @since 3.1
     */
     ...
     *
     * @param rule The rule to apply to the background job or <code>null if there isn't one.
     * @param jobFamilies the families the job should belong to or <code>null if none.
     *
     * @since 3.1
     */
    public void runInBackground(ISchedulingRule rule, final Object[] jobFamilies) {
        ...
        Job job = new WorkspaceJob(removeMnemonics(getText())) {
             ...
             * (non-Javadoc)
             *
             * @see org.eclipse.core.resources.WorkspaceJob#runInWorkspace(org.eclipse.core.runtime.
             * IProgressMonitor)
             */
    ...
    /**
     * Returns the operation to perform when this action runs. The returned operation must be an
     * {@link IRunnableWithProgress} that will perform the action's work. The default implementation
     * returns an operation that will iterate over the selected resources and call
     * {@link #invokeOperation(IResource, IProgressMonitor)} for each resource. Subclasses must
     * either implement {@link #invokeOperation(IResource, IProgressMonitor)} or override this
     * method to provide a different operation. Subclasses typically override this method when an
     * undoable operation is to be provided.
     *
     * @param errorStatus an array of error status objects to which the result of running the
