...
public class RenameResourceAction extends WorkspaceAction {
    ...
    /*
     * The tree editing widgets. If treeEditor is null then edit using the dialog. We keep the
     * editorText around so that we can close it if a new selection is made.
     */
    ...
    /**
     * Creates a new action. Using this constructor directly will rename using a dialog rather than
     * the inline editor of a ResourceNavigator.
     *
     ...
     * the inline editor of a ResourceNavigator.
     *
     * @param provider the IShellProvider for any dialogs
     * @since 3.4
     */
     ...
     * Creates a new action.
     *
     * @param provider the shell provider for any dialogs
     * @param tree the tree
     * @since 3.4
     */
    ...
    /**
     * Returns the elements that the action is to be performed on. Return the resource cached by the
     * action as we cannot rely on the selection being correct for inlined text.
     *
    ...
    /**
     * Return the new name to be given to the target resource or <code>null<code> if the query was
     * canceled. Rename the currently selected resource using the table editor. Continue the action
    ...
    /*
     * Run the receiver using an inline editor from the supplied navigator. The navigator will tell
     * the action when the path is ready to run.
     */
    ...
    /**
     * Return the currently selected resource. Only return an IResouce if there is one and only one
     * resource selected.
     *
     ...
     * <code>SelectionListenerAction</code> method ensures that this action is disabled if any of
     * the selections are not resources or resources that are not local.
     */
    @Override
