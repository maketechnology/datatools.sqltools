...
/**
 * A {@link DecoratingStyledCellLabelProvider} is a {@link DelegatingStyledCellLabelProvider} that
 * uses a nested {@link DelegatingStyledCellLabelProvider.IStyledLabelProvider} to compute styled
 * text label and image and takes a {@link ILabelDecorator} to decorate the label.
 *
 * <p>
 * Use this label provider as a replacement for the {@link DecoratingLabelProvider} when decorating
 * styled text labels.
 * </p>
 *
 * <p>
 * The {@link DecoratingStyledCellLabelProvider} will try to evaluate the text decoration added by
 * the {@link ILabelDecorator} and will apply the style returned by
 * {@link #getDecorationStyle(Object)}
 * </p>
 * <p>
 * The {@link ILabelDecorator} can optionally implement {@link IColorDecorator} and
 * {@link IFontDecorator} to provide foreground and background color and font decoration.
 * </p>
 *
 * @since 1.1
 */
public class DecoratingStyledCellLabelProvider extends DelegatingStyledCellLabelProvider {
    ...
    /**
     * Creates a {@link DecoratingStyledCellLabelProvider} that delegates the requests for styled
     * labels and for images to a {@link DelegatingStyledCellLabelProvider.IStyledLabelProvider}.
     *
     ...
     * @param decorationContext a decoration context or <code>null</code> if the no decorator is
     *        configured or the default decorator should be used
     */
    ...
    /**
     * Set the decoration context that will be based to the decorator for this label provider if
     * that decorator implements {@link LabelDecorator}.
     *
    ...
    protected StyledString getStyledText(Object element) {
        ...
        if (decorated.length() > originalStart + label.length()) { // decorator
                                                                   // appended
                                                                   // something
            return styledString.append(decorated.substring(originalStart + label.length()), style);
        }
        return styledString;
    }

    /**
     * Sets the {@link StyledString.Styler} to be used for string decorations. By default the
     * {@link StyledString#DECORATIONS_STYLER decoration style}. Clients can override.
     *
     * Note that it is the client's responsibility to react on color changes of the decoration color
    ...
    /**
     * Sets the label decorator. Removes all known listeners from the old decorator, and adds all
     * known listeners to the new decorator. The old decorator is not disposed. Fires a label
     * provider changed event indicating that all labels should be updated. Has no effect if the
     * given decorator is identical to the current one.
     *
     * @param newDecorator the label decorator, or <code>null</code> if no decorations are to be
     *        applied
     */
