...
/**
 * A CopyResourcesOperation represents an undoable operation for copying one or more resources in
 * the workspace. Clients may call the public API from a background thread.
 *
 * <p>
 * This operation can track any overwritten resources and restore them when the copy is undone. It
 * is up to clients to determine whether overwrites are allowed. If a resource should not be
 * overwritten, it should not be included in this operation. In addition to checking for overwrites,
 * the target location for the copy is assumed to have already been validated by the client. It will
 * not be revalidated on undo and redo.
 * </p>
 *
 * <p>
 * This class is intended to be instantiated and used by clients. It is not intended to be
...
public class CopyResourcesOperation extends AbstractCopyOrMoveResourcesOperation {
    ...
    /**
     * Create a CopyResourcesOperation that copies a single resource to a new location. The new
     * location includes the name of the copy.
     *
     * @param resource the resource to be copied
     * @param newPath the new workspace-relative path for the copy, including its desired name.
    ...
    /**
     * Create a CopyResourcesOperation that copies all of the specified resources to a single target
     * location. The original resource name will be used when copied to the new location.
     *
     * @param resources the resources to be copied
     * @param destinationPath the workspace-relative destination path for the copied resource.
     * @param label the label of the operation
     */
    ...
    /**
     * Create a CopyResourcesOperation that copies each of the specified resources to its
     * corresponding destination path in the destination path array. The resource name for the
     * target is included in the corresponding destination path.
     *
     * @param resources the resources to be copied. Must not contain null resources.
     * @param destinationPaths a workspace-relative destination path for each copied resource, which
     *        includes the name of the resource at the new destination. Must be the same length as
     *        the resources array, and may not contain null paths.
     * @param label the label of the operation
     */
    ...
    /**
     * Move or copy any known resources according to the destination parameters known by this
     * operation. Store enough information to undo and redo the operation.
     *
     * @param monitor the progress monitor to use for the operation
     * @param uiInfo the IAdaptable (or <code>null</code>) provided by the caller in order to supply
     *        UI information for prompting the user if necessary. When this parameter is not
     *        <code>null</code>, it contains an adapter for the org.eclipse.swt.widgets.Shell.class
     * @throws CoreException propagates any CoreExceptions thrown from the resources API
     */
    protected void copy(IProgressMonitor monitor, IAdaptable uiInfo) throws CoreException {
        ...
        for (int i = 0; i < resources.length; i++) {
            ...
            // overwrites = WorkspaceUndoUtil.copy(new IResource[] { resources[i] },
            // getDestinationPath(resources[i], i),
            // resourcesAtDestination, subMonitor.split(1), uiInfo, true, fCreateGroups,
            // fCreateLinks,
            // fRelativeToVariable);
    ...
    /*
     * This implementation deletes the previously made copies and restores any resources that were
    ...
    /*
     * This implementation computes the ability to delete the original copy and restore any
    ...
    /*
     * Record the original resources, including a resource description to describe it. This is so we
     * can make sure the original resources and their subtrees are intact before allowing a copy to
     * be undone.
     */
