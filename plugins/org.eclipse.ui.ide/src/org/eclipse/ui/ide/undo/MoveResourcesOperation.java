...
/**
 * A MoveResourcesOperation represents an undoable operation for moving one or more resources in the
 * workspace. Clients may call the public API from a background thread.
 * <p>
 * This operation can track any overwritten resources and restore them when the move is undone. It
 ...
 * the target location for the move is assumed to have already been validated by the client. It will
...
public class MoveResourcesOperation extends AbstractCopyOrMoveResourcesOperation {
    ...
    /**
     * Create a MoveResourcesOperation that moves all of the specified resources to the same target
     * location, using their existing names.
     * 
     * @param resources the resources to be moved
    ...
    /**
     * Create a MoveResourcesOperation that moves a single resource to a new location. The new
     * location includes the name of the resource, so this may be used for a move/rename operation
     * or a simple move.
     * 
     * @param resource the resource to be moved
     * @param newPath the new path for the resource, including its desired name.
     * @param label the label of the operation
     */
     ...
     * Store enough information to undo and redo the operation.
     * 
    ...
    protected void move(IProgressMonitor monitor, IAdaptable uiInfo) throws CoreException {
        ...
        for (int i = 0; i < resources.length; i++) {
            ...
            overwrites = WorkspaceUndoUtil.move(new IResource[] {resources[i]}, getDestinationPath(resources[i], i), resourcesAtDestination, undoDestinationPaths, new SubProgressMonitor(monitor, 1000 / resources.length), uiInfo, true);
        ...
        if (resourceDescriptions != null) {
            for (int i = 0; i < resourceDescriptions.length; i++) {
                if (resourceDescriptions[i] != null) {
                    resourceDescriptions[i].createResource(new SubProgressMonitor(monitor, 1000 / resourceDescriptions.length));
                }
            }
        }
