...
/**
 * An AbstractCopyOrMoveResourcesOperation represents an undoable operation for moving or copying
 * one or more resources in the workspace. Clients may call the public API from a background thread.
 *
...
abstract class AbstractCopyOrMoveResourcesOperation extends AbstractResourcesOperation {
    ...
    /**
     * Create an AbstractCopyOrMoveResourcesOperation that moves or copies all of the specified
     * resources to the specified paths. The destination paths must include the names of the
     * resources at their new location.
     *
     * @param resources the resources to be moved or copied. May not contain null resources, or
     *        resources that are descendants of already included resources.
     * @param destinationPaths the destination paths for the resources, including the name to be
     *        assigned to the resource at its new location. May not contain null paths, and must be
     *        the same length as the resources array.
    ...
    /**
     * Create an AbstractCopyOrMoveResourcesOperation that moves or copies all of the specified
     * resources to the same target location, using their existing names.
     *
     * @param resources the resources to be moved or copied
     * @param destinationPath the destination path for the resources, not including the name of the
     *        new resource.
     * @param label the label of the operation
     */
    ...
    /**
     * Create an AbstractCopyOrMoveResourcesOperation whose destination is not yet specified.
     *
     * @param resources the resources to be modified
     * @param label the label of the operation
     */
    ...
    /**
     * Compute the status for moving or copying the resources. A status severity of <code>OK
     * indicates that the copy or move is likely to be successful. A status severity of <code>ERROR
     * indicates that the operation is no longer valid. Other status severities are open to
     * interpretation by the caller.
     *
     * Note this method may be called on initial moving or copying of a resource, or when a move or
     * copy is undone or redone. Therefore, this method should check conditions that can change over
     * the life of the operation, such as whether the file to moved or copied exists, and whether
     * the target location is still valid. One-time static checks should typically be done by the
     * caller so that the user is not continually prompted or warned about conditions that were
     * acceptable at the time of original execution and do not change over time.
     *
     * @return the status indicating the projected outcome of moving or copying the resources.
     */
    ...
    /**
     * Return the destination path that should be used to move or copy the specified resource. This
     * path is relative to the workspace.
     *
     * @param resource the resource being moved or copied
     * @param index the integer index of the resource in the resource array
     ...
     * (non-Javadoc)
     *
     * @see org.eclipse.ui.ide.undo.AbstractWorkspaceOperation#appendDescriptiveText(java.lang.
     * StringBuffer)
     */
     ...
     *
     * @param resource the resource whose path is to be checked
     * @param index the integer index of the resource in the resource array
     ...
     *
     * @see
     * org.eclipse.ui.ide.undo.AbstractWorkspaceOperation#computeExecutionStatus(org.eclipse.core.
     ...
     *
     * @see
     * org.eclipse.ui.ide.undo.AbstractWorkspaceOperation#computeRedoableStatus(org.eclipse.core.
     * runtime.IProgressMonitor)
     */
