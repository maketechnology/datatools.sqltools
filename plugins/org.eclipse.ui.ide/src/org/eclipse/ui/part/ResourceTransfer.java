...
/**
 * The <code>ResourceTransfer</code> class is used to transfer an array of <code>IResource</code>s
 * from one part to another in a drag and drop operation or a cut, copy, paste action.
 * <p>
 * In every drag and drop operation there is a <code>DragSource</code> and a <code>DropTarget</code>
 * . When a drag occurs a <code>Transfer</code> is used to marshal the drag data from the source
 * into a byte array. If a drop occurs another <code>Transfer</code> is used to marshal the byte
 * array into drop data for the target.
 * </p>
 * <p>
 * When a <code>CutAction</code> or a <code>CopyAction</code> is performed, this transfer is used to
 * place references to the selected resources on the <code>Clipboard</code>. When a
 * <code>PasteAction</code> is performed, the references on the clipboard are used to move or copy
 * the resources to the selected destination.
 * </p>
 * <p>
 * This class can be used for a <code>Viewer<code> or an SWT component directly.
 * A singleton is provided which may be serially reused (see <code>getInstance</code>). It is not
...
public class ResourceTransfer extends ByteArrayTransfer {
    ...
    /**
     * See bug 205678: sometimes we can misinterpret native data received from clipboard. No one
     * seriously would copy/paste or drag/drop more then 100.000 resources: only creating an *empty*
     * array of 100.000.000 resources will cause OOME on 512 MB heap size (default for shipped
     * Eclipse packages), same with copy/paste of a *full* array of 10.000.000 elements.
     */
    ...
    public void javaToNative(Object data, TransferData transferData) {
        ...
        /**
         * The resource serialization format is: (int) number of resources Then, the following for
         * each resource: (int) resource type (String) path of resource
         */
    ...
    public Object nativeToJava(TransferData transferData) {
        ...
        try {
            ...
            if (count > MAX_RESOURCES_TO_TRANSFER) {
                ...
                IDEWorkbenchPlugin.log(message, new IllegalArgumentException("Maximum limit of resources to transfer is: " + MAX_RESOURCES_TO_TRANSFER)); //$NON-NLS-1$
                return null;
            }
